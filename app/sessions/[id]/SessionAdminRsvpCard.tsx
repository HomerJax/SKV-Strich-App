...
function statusLabel(
  player: Player,
  labels: { out: string; in: string; open: string },
) {
  const status = rsvpStatus(player);
  if (status === "out") return labels.out;
  if (status === "in") return labels.in;
  return labels.open;
}

function rosterName(player: EventRosterPlayer, fallback: string) {
  return player.nickname?.trim() || [player.first_name, player.last_name].filter(Boolean).join(" ").trim() || fallback;
}
...
export default function SessionAdminRsvpCard({ sessionId, players, hasResult, isAdmin }: Props) {
  ...
  async function updateRsvp(playerId: number, status: AdminRsvpStatus) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload?.error || t("rsvp.changeFailed"));
      if (typeof window !== "undefined") {
        window.sessionStorage.setItem(ATTENDANCE_SCROLL_KEY, String(window.scrollY));
        window.location.reload();
      }
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : t("rsvp.changeFailed"));
      ...
      {absentPlayers.length > 0 ? (
        <section className="rounded-[20px] border border-rose-200 bg-rose-50/60 p-4 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-sm font-black text-slate-950">{t("rsvp.absences")}</div>
              <div className="mt-0.5 text-[11px] text-slate-500">{t("rsvp.absenceReasonsHint")}</div>
            </div>
              ...
              <div key={player.id} className="flex items-start justify-between gap-3 rounded-xl border border-rose-100 bg-white px-3 py-2">
                <div className="min-w-0">
                  ...
                  <div className={`mt-0.5 text-[11px] leading-4 ${player.rsvp_reason ? "font-semibold text-rose-700" : "text-slate-400"}`}>
                    {player.rsvp_reason ? `„${player.rsvp_reason}“` : t("rsvp.noReason")}
                  </div>
                </div>
                <span className="shrink-0 rounded-full bg-rose-100 px-2 py-1 text-[9px] font-black text-rose-700">{t("rsvp.declined")}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="rounded-[20px] border border-violet-200 bg-violet-50/60 p-4 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-[11px] leading-5 text-slate-600">{t("rsvp.rosterDefaultHint")}</p>
              <div className="mt-2 text-[10px] font-bold text-violet-700">{t("rsvp.rosterCount", { nominated: nominatedCount, total: rosterPlayers.length })}</div>
            </div>
            <button type="button" onClick={() => setRosterOpen((value) => !value)} disabled={rosterLoading} className="shrink-0 rounded-xl bg-slate-950 px-3 py-2 text-xs font-black text-white disabled:opacity-50">
              {rosterOpen ? t("rsvp.close") : t("rsvp.setRoster")}
            </button>
          </div>
          {rosterOpen ? (
      ...
      {isAdmin && !hasResult && eligiblePlayers.length > 0 ? (
        <section className="rounded-[20px] border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <div className="text-sm font-bold text-slate-950">{t("rsvp.manage")}</div>
              <p className="mt-1 text-[11px] leading-5 text-slate-500">{t("rsvp.manageHint")}</p>
            </div>
            {absentPlayers.length > 0 ? <span className="rounded-full bg-rose-50 px-2.5 py-1 text-[11px] font-semibold text-rose-700 ring-1 ring-rose-200">{t("rsvp.declinedCount", { count: absentPlayers.length })}</span> : null}
          </div>
          <div className="mt-3 flex flex-col gap-2 sm:flex-row">
            <select value={selectedPlayerId} onChange={(event) => { setSelectedPlayerId(event.target.value); setError(null); }} disabled={busyPlayerId !== null} className="min-h-10 min-w-0 flex-1 rounded-xl border border-slate-300 bg-white px-3 text-sm text-slate-900 outline-none transition focus:border-slate-500 disabled:opacity-60">
              <option value="">{t("rsvp.selectPlayer")}</option>
              {eligiblePlayers.map((player) => <option key={player.id} value={player.id}>{getPlayerDisplayName(player)} · {statusLabel(player, { out: t("rsvp.statusOut"), in: t("rsvp.statusIn"), open: t("rsvp.statusOpen") })}</option>)}
            </select>
            <button type="button" disabled={!selectedPlayer || busyPlayerId !== null} onClick={() => { if (selectedPlayer) void updateRsvp(selectedPlayer.id, selectedIsAbsent ? "open" : "out"); }} className={`inline-flex min-h-10 items-center justify-center rounded-xl px-4 text-sm font-bold transition disabled:cursor-not-allowed disabled:opacity-50 ${selectedIsAbsent ? "border border-slate-300 bg-white text-slate-800" : "bg-rose-600 text-white"}`}>
              {selectedPlayer && busyPlayerId === selectedPlayer.id ? t("rsvp.saving") : selectedIsAbsent ? t("rsvp.undoDecline") : t("rsvp.declinePlayer")}
            </button>
          </div>
        </section>
      ) : null}
