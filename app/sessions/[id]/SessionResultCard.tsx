...
export default function SessionResultCard({
  ...
  const isPhotoCard = showPhotoSection && !showResultSection;
  const isResultCard = showResultSection && !showPhotoSection;

  const isDone = isResultCard
    ? hasResult
    : isPhotoCard
      ? Boolean(hasWinnerPhoto || previewUrl)
      : false;

  const label = useMemo(() => {
    if (isResultCard) return "Ergebnis";
    if (isPhotoCard) return "Siegerfoto";
    return "Session-Abschluss";
  }, [isPhotoCard, isResultCard]);

  const heading =
    title ?? (isResultCard ? "Ergebnis" : isPhotoCard ? "Siegerfoto" : "Abschluss");

  const text =
    description ??
    (isResultCard
      ? "Ergebnis speichern, ändern oder löschen."
      : isPhotoCard
      ...
      <section className="rounded-[24px] border border-black/10 bg-white shadow-sm">
        <button
          ...
          className={`flex w-full items-center justify-between gap-4 rounded-[24px] px-4 py-4 text-left ${
            isDone ? "border border-emerald-200 bg-emerald-50" : ""
          }`}
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className={`text-base font-bold ${isDone ? "text-emerald-950" : "text-slate-950"}`}>
                {isResultCard
                  ? isDone
                    ? "Ergebnis erledigt"
                    : heading
    ...
    <section className="rounded-[24px] border border-black/10 bg-white shadow-sm">
      <div className="flex items-start justify-between gap-4 px-5 py-5">
        <div className="min-w-0">
          ...
          <h2 className="mt-1 text-lg font-bold text-slate-950 sm:text-xl">{heading}</h2>
