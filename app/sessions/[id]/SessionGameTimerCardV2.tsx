...
type PersistedTimerRuntime = {
  ...
  finalAt: number | null;
  halftimeSignalHandled: boolean;
  settings: GameTimerSettings;
  nativeHalftimeScheduled: boolean;
  nativeFinalScheduled: boolean;
  savedAt: number;
};
...
const HALFTIME_SIGNAL_MS = 10_000;
...
function normalizeSettings(settings: GameTimerSettings): GameTimerSettings {
  return {
    ...settings,
    halftimeBehavior: settings.halftimeBehavior === "signal" ? "signal" : "pause",
  };
}
...
function getInitialRuntime(settings: GameTimerSettings): PersistedTimerRuntime {
  ...
  return {
    phase: "idle",
    segment:
      normalized.halftimeEnabled && normalized.halftimeBehavior === "pause"
        ? "first"
        : "continuous",
    targetAt: null,
    endAt: null,
    halftimeAt: null,
    finalAt: null,
    halftimeSignalHandled: false,
    settings: normalized,
    nativeHalftimeScheduled: false,
    nativeFinalScheduled: false,
    savedAt: Date.now(),
  };
}
...
export default function SessionGameTimerCard({
  ...
  const stopBrowserAlarm = useCallback(() => {
    if (typeof window !== "undefined" && signalStopTimeoutRef.current !== null) {
      window.clearTimeout(signalStopTimeoutRef.current);
      signalStopTimeoutRef.current = null;
    }
    stopRepeatingTimerAlarm();
  }, []);
  ...
  const resetRuntime = useCallback(
    ...
    [cancelAllNativeAlarms, persistRuntime, settings, stopBrowserAlarm],
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (validAge && validPhase && parsed.settings) {
        const normalizedRuntime: PersistedTimerRuntime = {
          ...parsed,
          settings: normalizeSettings(parsed.settings),
          halftimeAt: parsed.halftimeAt ?? null,
          finalAt: parsed.finalAt ?? parsed.endAt ?? parsed.targetAt ?? null,
          halftimeSignalHandled: parsed.halftimeSignalHandled ?? false,
          nativeHalftimeScheduled: parsed.nativeHalftimeScheduled ?? false,
          nativeFinalScheduled: parsed.nativeFinalScheduled ?? false,
        };
        setRuntime(normalizedRuntime);
        if (normalizedRuntime.phase === "running" && normalizedRuntime.targetAt) {
          setRemainingMs(Math.max(0, normalizedRuntime.targetAt - Date.now()));
        }
      } else {
  ...
  useEffect(() => {
    if (
      ...
      !runtime.settings.halftimeEnabled ||
      runtime.settings.halftimeBehavior !== "signal" ||
      runtime.halftimeSignalHandled ||
      !runtime.halftimeAt ||
      Date.now() < runtime.halftimeAt
    ) {
      return;
    }
    ...
    if (!runtime.nativeHalftimeScheduled) {
      void startRepeatingTimerAlarm(runtime.settings.alarmSound).then((started) => {
        ...
        signalStopTimeoutRef.current = window.setTimeout(() => {
          stopRepeatingTimerAlarm();
          signalStopTimeoutRef.current = null;
        }, HALFTIME_SIGNAL_MS);
      });
  ...
  useEffect(() => {
    ...
    const isPauseHalftime =
      runtime.settings.halftimeEnabled &&
      runtime.settings.halftimeBehavior === "pause" &&
      runtime.segment === "first";
    ...
    if (!runtime.nativeFinalScheduled) {
      void startRepeatingTimerAlarm(runtime.settings.alarmSound);
    }
    persistRuntime({
      ...runtime,
  ...
  const displayTime = useMemo(() => {
    ...
    if (runtime.phase === "idle") {
      if (settings.mode === "duration") {
        const milliseconds =
          settings.halftimeEnabled && settings.halftimeBehavior === "pause"
  ...
  async function scheduleNativeAlarm(params: {
    key: string;
    atEpochMs: number;
    kind: "halftime" | "final";
    persistent: boolean;
    ...
    return scheduleNativeGameTimerAlarm(params);
  }
  ...
  async function startTimer() {
    ...
    let targetAt: number;
    let segment: TimerSegment = "continuous";

    if (activeSettings.mode === "duration") {
      const totalMs = activeSettings.durationMinutes * 60 * 1000;
      if (activeSettings.halftimeEnabled) {
        halftimeAt = now + totalMs / 2;
      }

      if (
        activeSettings.halftimeEnabled &&
        activeSettings.halftimeBehavior === "pause"
      ) {
        targetAt = halftimeAt!;
        segment = "first";
      } else {
    ...
    let nativeHalftimeScheduled = false;
    let nativeFinalScheduled = false;

    if (nativeReady.useNative) {
      if (activeSettings.halftimeEnabled && halftimeAt) {
        nativeHalftimeScheduled = await scheduleNativeAlarm({
          key: halftimeAlarmKey,
          atEpochMs: halftimeAt,
          kind: "halftime",
          sound: activeSettings.alarmSound,
          persistent: activeSettings.halftimeBehavior === "pause",
        });
      ...
      if (finalAt) {
        nativeFinalScheduled = await scheduleNativeAlarm({
          key: finalAlarmKey,
          atEpochMs: finalAt,
          kind: "final",
          sound: activeSettings.alarmSound,
    ...
    persistRuntime({
      ...
      nativeHalftimeScheduled,
      nativeFinalScheduled,
      savedAt: Date.now(),
    });
  ...
  async function startSecondHalf() {
    ...
    let nativeFinalScheduled = runtime.nativeFinalScheduled;
    if (nativeAlarmSupported && !nativeFinalScheduled) {
      const nativeReady = await ensureNativeAlarmReady();
      if ("blocked" in nativeReady && nativeReady.blocked) return;
      if (nativeReady.useNative) {
        nativeFinalScheduled = await scheduleNativeAlarm({
                ...
                <div className="mt-2 grid gap-2">
                  {GAME_TIMER_HALFTIME_BEHAVIOR_OPTIONS.map((option) => (
                    <label key={option.value} className="flex items-start gap-3 rounded-xl border border-slate-100 p-2.5">
                      <input
                        type="radio"
                        name={`session-halftime-${sessionId}`}
                        checked={settings.halftimeBehavior === option.value}
                        onChange={() =>
                          setSettings((current) => ({
                            ...current,
                            halftimeBehavior: option.value as GameTimerHalftimeBehavior,
                          }))
                        }
