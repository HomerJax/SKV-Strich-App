...
export async function POST(
  ...
  if (!hasAdminAccess) return fail("Nur Admins dürfen Rückmeldungen anderer Spieler ändern.", 403);
  ...
  if (status !== "out" && status !== "open") return fail("Ungültiger Rückmeldestatus.", 400);
  ...
  if (!player) return fail("Spieler gehört nicht zu diesem Team.", 404);
  if (resultError) return fail(`Ergebnisstatus konnte nicht geprüft werden: ${resultError.message}`, 500);
  if (result) return fail("Rückmeldungen sind gesperrt, weil bereits ein Ergebnis gespeichert ist.", 400);
  ...
  if (teamIds.length > 0) {
    ...
    if (teamPlayerError) return fail(`Team-Zuordnung konnte nicht entfernt werden: ${teamPlayerError.message}`, 500);
  }

  const { error: presenceError } = await adminSupabase.from("session_players").delete().eq("session_id", sessionId).eq("player_id", playerId);
  if (presenceError) return fail(`Anwesenheit konnte nicht aktualisiert werden: ${presenceError.message}`, 500);
  ...
  if (status === "open") {
    ...
    if (clearError) return fail(`Absage konnte nicht aufgehoben werden: ${clearError.message}`, 500);
    return ok({ message: "Absage aufgehoben. Rückmeldung ist wieder offen.", playerId, status: "open" });
  }
  ...
  if (rsvpError) return fail(`Absage konnte nicht gespeichert werden: ${rsvpError.message}`, 500);

  const playerName = player.nickname?.trim() || [player.first_name?.trim(), player.last_name?.trim()].filter(Boolean).join(" ") || player.name?.trim() || "Spieler";
  return ok({ message: `${playerName} wurde für diese Session abgesagt.`, playerId, status: "out" });
}
