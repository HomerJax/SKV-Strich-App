...
export async function POST(
  ...
  if (!isAdmin) {
    return fail("Nur Admins dürfen den Event-Kader festlegen.", 403);
  }

  if (session.type !== "event") {
    return fail("Ein Event-Kader kann nur bei Terminen gepflegt werden.", 400);
  }

  const body = (await request.json().catch(() => null)) as
    | { playerId?: number; nominated?: boolean }
    | null;
  ...
  if (!player || player.is_active === false || player.is_guest === true) {
    return fail("Spieler ist für den Event-Kader nicht verfügbar.", 400);
  }
  ...
  if (nominated) {
    ...
    if (error) {
      return fail(`Event-Kader konnte nicht aktualisiert werden: ${error.message}`, 500);
    }

    return ok({ message: "Spieler ist wieder im Event-Kader.", playerId, nominated: true });
  }

  const { error: exclusionError } = await adminSupabase
    .from("session_event_exclusions")
    .upsert(
      {
        club_id: clubId,
  ...
  if (exclusionError) {
    return fail(`Event-Kader konnte nicht aktualisiert werden: ${exclusionError.message}`, 500);
  }
  ...
  if (presenceError || rsvpError) {
    return fail(
      `Spieler wurde aus dem Event-Kader entfernt, aber die Rückmeldung konnte nicht vollständig bereinigt werden: ${presenceError?.message ?? rsvpError?.message}`,
      500,
    );
  }
