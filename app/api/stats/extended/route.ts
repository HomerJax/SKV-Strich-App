...
type TeammateStats = { playerId: number; name: string; games: number; wins: number; losses: number; draws: number };
...
export async function GET(request: Request) {
  ...
  const [{ data: playerTeamRows, error: playerTeamError }, { data: seasonsData, error: seasonsError }] = await Promise.all([
  ...
  if (playerTeamError) return NextResponse.json({ error: playerTeamError.message }, { status: 500 });
  if (seasonsError) return NextResponse.json({ error: seasonsError.message }, { status: 500 });

  const teamIds = Array.from(new Set((playerTeamRows ?? []).map((row) => Number(row.team_id)).filter(Number.isFinite)));
  if (teamIds.length === 0) return NextResponse.json({ enabled: true, bestMonth: null, teammates: { mostPlayed: [], mostWins: [], mostLosses: [] } });

  const { data: teamsData, error: teamsError } = await supabase.from("teams").select("id, session_id").in("id", teamIds);
  if (teamsError) return NextResponse.json({ error: teamsError.message }, { status: 500 });
  const teams = (teamsData ?? []) as TeamRow[];
  const sessionIds = Array.from(new Set(teams.map((team) => team.session_id)));
  if (sessionIds.length === 0) return NextResponse.json({ enabled: true, bestMonth: null, teammates: { mostPlayed: [], mostWins: [], mostLosses: [] } });

  const { data: sessionsData, error: sessionsError } = await supabase.from("sessions").select("id, date, season_id").eq("club_id", clubId).in("id", sessionIds);
  ...
  const scopedSessions = scope === "career" ? allSessions : currentSeason ? allSessions.filter((session) => session.season_id === currentSeason.id) : [];
  ...
  if (scopedTeamIds.length === 0) return NextResponse.json({ enabled: true, bestMonth: null, teammates: { mostPlayed: [], mostWins: [], mostLosses: [] } });

  const [{ data: teammateRowsData, error: teammateRowsError }, { data: resultsData, error: resultsError }] = await Promise.all([
    supabase.from("team_players").select("team_id, player_id").in("team_id", scopedTeamIds),
    supabase.from("results").select("session_id, team_a_id, team_b_id, goals_team_a, goals_team_b").eq("club_id", clubId).in("session_id", Array.from(scopedSessionIds)),
  ]);
  if (teammateRowsError) return NextResponse.json({ error: teammateRowsError.message }, { status: 500 });
  ...
  const teammateIds = Array.from(new Set(teammateRows.map((row) => row.player_id).filter((id) => id !== player.id && Number.isFinite(id))));
  const { data: playersData, error: playersError } = teammateIds.length
    ? await supabase.from("players").select("id, name, first_name, last_name, nickname").eq("club_id", clubId).in("id", teammateIds)
  ...
  const monthStats = new Map<string, { month: string; wins: number; losses: number; draws: number; games: number }>();
  const teammatesByTeam = new Map<number, number[]>();
  for (const row of teammateRows) {
    if (row.player_id === player.id) continue;
    const ids = teammatesByTeam.get(row.team_id) ?? [];
    ids.push(row.player_id);
    teammatesByTeam.set(row.team_id, ids);
  }

  for (const result of (resultsData ?? []) as ResultRow[]) {
    const myTeamId = myTeamBySession.get(result.session_id);
    if (!myTeamId || (result.team_a_id !== myTeamId && result.team_b_id !== myTeamId)) continue;
    const outcome = getOutcome(result, myTeamId);
    const monthKey = sessionById.get(result.session_id)?.date?.slice(0, 7);
    if (monthKey) {
      const month = monthStats.get(monthKey) ?? { month: monthKey, wins: 0, losses: 0, draws: 0, games: 0 };
      month.games += 1;
    ...
    for (const teammateId of teammatesByTeam.get(myTeamId) ?? []) {
  ...
  return NextResponse.json({
    ...
    bestMonth,
    teammates: {
      mostPlayed: top3("games"),
      mostWins: top3("wins"),
      mostLosses: top3("losses"),
    },
  });
}
