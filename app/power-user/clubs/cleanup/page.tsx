...
export default async function PowerUserClubCleanupPage({
  ...
  const [
    ...
    admin
      .from("clubs")
      .select("id, display_name, name, created_at, deleted_at, purge_after")
  ...
  const players = playersResult.error
    ? []
  ...
  const views: ClubCleanupView[] = clubs.map((club) => {
  ...
  const activeViews = views
    .filter((view) => !view.club.deleted_at)
    .sort((a, b) => {
  ...
  const likelyTestCount = activeViews.filter(
    (view) => view.status === "test/leer"
  ).length;
  const lowUsageCount = activeViews.filter(
    (view) => view.status === "kaum genutzt"
  ).length;
  const deleteError =
  ...
  const deletedName =
    typeof params.deleted_name === "string" ? params.deleted_name : "";
  const restored = params.restored === "1";
  const restoredName =
    typeof params.restored_name === "string" ? params.restored_name : "";
  const restoreError = typeof params.restore_error === "string";
  ...
  return (
    <main className="min-h-screen bg-neutral-100">
      <section className="mx-auto flex w-full max-w-6xl flex-col gap-5 px-4 py-5 sm:px-6 lg:px-8">
        ...
        {trashViews.length > 0 ? (
          <section className="rounded-[28px] border border-violet-200 bg-violet-50/40 p-5">
              ...
              {trashViews.map((view) => (
                ...
                >
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <form method="post" action="/power-user/clubs/restore">
              ...
              <div className="border-t border-slate-100 p-5">
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4">
                    ...
                    <form
                      method="post"
                      action="/power-user/clubs/delete"
