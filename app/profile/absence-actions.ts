...
export async function createAbsenceAction(formData:FormData){
 ...
 const {clubId,player}=await requireClub(); if(!player) redirect(`/profile?absence_error=${encodeURIComponent(t("absence.noProfile"))}`);
 ...
 if(!/^\d{4}-\d{2}-\d{2}$/.test(start)||!/^\d{4}-\d{2}-\d{2}$/.test(end)||end<start) redirect(`/profile?absence_error=${encodeURIComponent(t("absence.invalidRange"))}`);
 ...
 if(sessionsError) redirect(`/profile?absence_error=${encodeURIComponent(t("absence.sessionsLoadFailed"))}`); const sessionIds=(sessions??[]).map(s=>Number(s.id)).filter(Number.isFinite);
 const {data:absence,error}=await admin.from("player_absences").insert({club_id:clubId,player_id:player.id,start_date:start,end_date:end,reason,note}).select("id").single(); if(error||!absence) redirect(`/profile?absence_error=${encodeURIComponent(t("absence.saveFailed"))}`);
 if(sessionIds.length){const now=new Date().toISOString();const {error:rsvpError}=await admin.from("session_rsvps").upsert(sessionIds.map(session_id=>({club_id:clubId,session_id,player_id:player.id,status:"out",updated_at:now})),{onConflict:"session_id,player_id"});if(rsvpError){await admin.from("player_absences").delete().eq("id",absence.id);redirect(`/profile?absence_error=${encodeURIComponent(t("absence.rsvpSaveFailed"))}`);}await admin.from("session_players").delete().eq("player_id",player.id).in("session_id",sessionIds);}
