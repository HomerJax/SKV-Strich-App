...
type StandingRow = {
  ...
  sessions: number;
  rank: number;
  movement: number | null; // + hoch, - runter, 0 gleich, null = keine Vorwoche
};
...
function computeRanks(rows: { wins: number; sessions: number }[]) {
  // Ranking: wins DESC, sessions DESC (oder sessions ASC? – du hast bisher nach Siege/Teilnahmen sortiert.
  // Hier: Siege zuerst, dann Teilnahmen (mehr Teilnahmen = höher), wie du es wolltest fürs Ranking-Grid.
  // Falls du "weniger Teilnahmen besser" willst, sag Bescheid – dann drehen wir das.
  const sorted = [...rows]
    .map((r, idx) => ({ ...r, __i: idx }))
    .sort((a, b) => {
  ...
  // Tie-Ranks mit Sprüngen (1,2,2,4 …)
...
async function buildStandings({
  ...
  seasonId: number | "all";
  upToSessionId: number | null; // wenn gesetzt: nur Sessions <= diese Session (nach Datum)
}) {
  // 1) Spieler
  ...
  // 2) Sessions (nach Saison oder alle)
  ...
  if (upToSessionId != null) {
    // wir schneiden nach Datum der upTo-Session
  ...
  // 5) Teams → Spielerzuordnung (für Sieger ermitteln)
  ...
  for (const r of results) {
    ...
    // wenn kein Ergebnis (null), dann keine Siege vergeben
    if (ga == null || gb == null) continue;

    // unentschieden: keine Siege
    if (ga === gb) continue;

    const winnerTeamId =
      ga > gb ? r.team_a_id : r.team_b_id;
...
export default function StandingsPage() {
  ...
  useEffect(() => {
    async function loadSeasons() {
      try {
        ...
        // Default: höchste Saison (erste nach sort) – falls vorhanden
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Sessions der Saison, um "Vorwoche" zu bestimmen
        let sessionsQuery = supabase.from("sessions").select("id, date, season_id");
        ...
        // Aktuell: bis letzte Session (oder alle, falls keine Sessions)
        ...
        if (prevBase) {
          for (const r of prevBase) prevRankByPlayer.set(r.player_id, r.rank);
        }
        ...
        const enriched: StandingRow[] = currentBase
          .map((r) => {
            ...
            const movement =
              prevRank == null ? null : prevRank - r.rank; // + wenn hoch
            return { ...r, movement };
          })
          // Sortierung = nach Rank (wichtig für Anzeige)
          .sort((a, b) => a.rank - b.rank || a.name.localeCompare(b.name));

        setRows(enriched);

        // URL Query setzen (damit refresh stabil)
  ...
  const seasonOptions = useMemo(() => {
    // "aktuell" oben: seasons sind bereits start_date desc sortiert
    const opts = seasons.map((s) => ({
      value: String(s.id),
      label: s.name,
    }));

    // Ewige Tabelle als 2. Eintrag
  ...
  function movementView(m: number | null) {
    if (m == null) return { text: "–", cls: "text-slate-400" };
    if (m === 0) return { text: "–", cls: "text-slate-400" };
  ...
  async function exportPDF() {
    ...
    // Bildgröße passend skalieren
    const imgWidth = pageWidth - 40;
    const imgHeight = (canvas.height * imgWidth) / canvas.width;

    let y = 20;

    // wenn zu hoch: auf mehrere Seiten aufteilen
    if (imgHeight <= pageHeight - 40) {
      pdf.addImage(imgData, "PNG", 20, y, imgWidth, imgHeight);
    } else {
      // Multi-page: wir „schieben“ das Bild nach oben pro Seite
      let remaining = imgHeight;
      let offset = 0;

      while (remaining > 0) {
        pdf.addImage(imgData, "PNG", 20, y - offset, imgWidth, imgHeight);
                    ...
                    <tr key={r.player_id} className="border-t border-slate-100">
                      <td className="px-2 py-2 align-middle">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[11px] font-semibold ${mv.cls}`}>
                            {mv.text}
                          </span>
                        </div>
                      </td>
